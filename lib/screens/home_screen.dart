...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _onAppStateChanged() {
    // Update favorite count when app state changes
    if (mounted) {
      setState(() {
        totalFavorites = _appStateService.favoritesCount;
      });
    }
  }
  ...
  Future<void> _loadDashboardData() async {
    try {
      // Always try loading from Hive first for instant display
      ...
      // Display Hive data immediately if available
      if (hiveStakeholders.isNotEmpty && mounted) {
      ...
      try {
        ...
        // Update Hive cache with latest data
  ...
  void _updateCounts(List<Stakeholder> stakeholders) {
    ...
    // Load favorite count asynchronously (non-blocking)
    _loadFavoritesCount();
  }

  Future<void> _loadFavoritesCount() async {
    try {
      // Load from cache service (instant)
      final count = _cacheService.getFavoritesCount();

      // Update app state service
      _appStateService.setFavoritesCount(count);
                  ...
                  _buildDashboardCard(
                    ...
                    onTap: () {
                      ...
                      Navigator.of(context).push(MaterialPageRoute(
                        builder: (context) => const FavouriteScreenWithNavbar(),
                      ));
                    },
                  ),
                  _buildDashboardCard(
