...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
    ...
    if (user != null) {
      try {
        ...
        if (userDoc.exists) {
          ...
          // Load LGAs for the user's state
          if (state.isNotEmpty) {
  ...
  Future<void> _loadTotalStakeholderCount() async {
    try {
      ...
      print('Error loading total stakeholder count: $e');
    }
  }
  ...
  Stream<QuerySnapshot> _getFilteredStakeholders() {
    ...
    if (selectedLGA.isNotEmpty) {
      debugPrint('🔍 [FILTER-BUILD] Added LGA filter: $selectedLGA');
      query = query.where('LGA', isEqualTo: selectedLGA);  // Changed from 'lg' to 'LGA' to match Firestore
    }
    if (selectedWard.isNotEmpty) {
      query = query.where('ward', isEqualTo: selectedWard);
      debugPrint('🔍 [FILTER-BUILD] Added Ward filter: $selectedWard');
    }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Search field for admin (client-side filtering)
                    TextFormField(
                    ...
                    // LGA Dropdown with dynamic Firestore data
                    if (isLoadingLGAs)
                      const Padding(
                    ...
                    const SizedBox(height: 12),
                    // Ward Dropdown with dynamic Firestore data
                    if (isLoadingWards)
                      const Padding(
                        padding: EdgeInsets.all(16.0),
                        child: CircularProgressIndicator(),
            ...
            // Dashboard Info Card - Stakeholder Count
            Card(
              elevation: 0,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(16),
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  final filteredDocs = allDocs.where((docSnap) {
                    ...
                    // LGA filter (normalized comparison)
                    if (selectedLGA.isNotEmpty) {
                    ...
                    // Ward filter (normalized comparison)
                    if (selectedWard.isNotEmpty) {
                    ...
                    // Search query (client-side, case-insensitive)
                    if (_searchQuery.isNotEmpty) {
                      ...
                      if (!(name.contains(queryKey) || assoc.contains(queryKey) || phone.contains(queryKey) || lgaKey.contains(queryKey) || wardKey.contains(queryKey))) {
                        return false;
                      }
                    }
