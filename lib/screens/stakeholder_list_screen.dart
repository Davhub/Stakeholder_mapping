...
class _StakeholderListScreenState extends State<StakeholderListScreen> {
  ...
  Future<void> _loadStakeholders() async {
    // INSTANT load from cache - no loading spinner needed if cache is populated
    if (_cacheService.isCachePopulated()) {
      // Load from cache immediately
      final cachedStakeholders =
          _cacheService.getStakeholdersByState(userState ?? 'Lagos');
      if (mounted) {
        setState(() {
    ...
    try {
      ...
      // Update cache
      await _cacheService.cacheStakeholders(firestoreStakeholders);
  ...
  void _applyFilters() {
    setState(() {
      ...
      final results = _cacheService.searchStakeholders(
        query: searchQuery,
        lg: selectedLGA,
        ward: selectedWard,
      );
      ...
      if (widget.initialFilter == 'association' && widget.filterValue != null) {
        filteredStakeholders =
            results.where((s) => s.association == widget.filterValue).toList();
      } else if (widget.initialFilter == 'ward' && widget.filterValue != null) {
        filteredStakeholders =
            results.where((s) => s.ward == widget.filterValue).toList();
      } else {
